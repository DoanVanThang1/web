...
<?php 
	...
	*/
	class product
	{
		...
		public function show_product($i)
		{ 
			$query = 
			"SELECT tbl_product.*, tbl_category.catName, tbl_brand.brandName
			 ...
			 order by tbl_product.productId desc LIMIT "."$i".", 5 ";
		...
		{
			$query="SELECT count(productId) as total from tbl_product ";
			$result = $this->db->select($query);
			return $result;
		}
		...
		public function search_product($str){
			$query = "SELECT * FROM tbl_product WHERE LOWER (productName) LIKE '%$str%' order by 'productId'";
			$result = $this->db->select($query);
			return $result;
		}
